...
public class TodoItemRepository : ITodoItemRepository 
{
    ...
    public Task<Entities.TodoItem[]> LoadEntitiesAsync(IEnumerable<TodoItemId> ids, CancellationToken cancellationToken = default(CancellationToken))
    {
        ...
        return _dbContext.TodoItems.Include(i => i.TodoList).ThenInclude(l => l.User).Where(item => guids.Contains(item.Id) && item.DeletedAt == null && item.TodoList.DeletedAt == null).OrderBy(item => item.CreatedAt).ToArrayAsync(cancellationToken);  
    }
